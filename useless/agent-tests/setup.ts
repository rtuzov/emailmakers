...
beforeAll(() => {
  ...
  jest.mock('@openai/agents', () => ({
    ...
    run: jest.fn().mockResolvedValue('Mock AI Response'),
    tool: jest.fn().mockImplementation((config) => config),
    generateTraceId: jest.fn().mockReturnValue('test-trace-id-123456'),
    getCurrentTrace: jest.fn().mockReturnValue({ id: 'test-trace-id-123456' })
  }));
  ...
  jest.mock('../../shared/utils/model-config', () => ({
    getUsageModel: jest.fn().mockReturnValue('gpt-4o-mini')
  }));
