...
jest.mock('@openai/agents', () => ({
  ...
  run: jest.fn().mockResolvedValue('Mock AI Response' as any),
  tool: jest.fn(),
  generateTraceId: jest.fn().mockReturnValue('test-trace-id'),
  getCurrentTrace: jest.fn()
}));

// Mock всех инструментов агентов
jest.mock('../tools/consolidated/context-provider', () => ({
...
jest.mock('../tools/consolidated/pricing-intelligence', () => ({
  pricingIntelligence: jest.fn().mockResolvedValue({
    success: true,
    data: { pricing: 'test' }
  }),
...
jest.mock('../tools/simple/content-create', () => ({
  contentCreate: jest.fn().mockResolvedValue({
    success: true,
    content_data: {
      complete_content: {
...
jest.mock('../tools/consolidated/email-renderer', () => ({
  emailRenderer: jest.fn().mockResolvedValue({
    success: true,
...
jest.mock('../tools/simple/html-validate', () => ({
  htmlValidate: jest.fn().mockResolvedValue({
    success: true,
...
jest.mock('../tools/simple/s3-upload', () => ({
...
describe('🔄 Agent Integration Pipeline Tests', () => {
  ...
  describe('📝➡️🎨 Content → Design Pipeline', () => {
    test('✅ Полный пайплайн Content → Design должен работать', async () => {
      ...
      expect(designResult.handoff_data).toBeDefined();
    ...
    test('🤖 AI коррекция должна спасать частично невалидные данные', async () => {
      ...
      const mockRun = jest.mocked(await import('@openai/agents')).run;
  ...
  describe('🔄 Full End-to-End Pipeline', () => {
    test('✅ Полный пайплайн Content → Design → Quality → Delivery', async () => {
      ...
      const qualityInput = {
        ...
        handoff_data: designResult.handoff_data
      };
  ...
  describe('📊 Performance Metrics Integration', () => {
    ...
    test('🎯 Агенты должны предоставлять информацию о возможностях', () => {
      ...
      // Проверяем структуру capabilities (возвращается объект)
      expect(contentCapabilities).toBeInstanceOf(Array);
      expect(designCapabilities).toBeInstanceOf(Object);
      expect(qualityCapabilities).toBeInstanceOf(Object);
      expect(deliveryCapabilities).toBeInstanceOf(Object);

      // Проверяем что capabilities содержат необходимые поля
      expect(contentCapabilities.length).toBeGreaterThan(0);
      expect(designCapabilities.agent_id).toBeDefined();
      expect(qualityCapabilities.agent_id).toBeDefined();
      expect(deliveryCapabilities.agent_id).toBeDefined();
    });
  });
});