...
type ContentGeneratorParams = {
  topic: string;
  action: 'generate' | 'optimize' | 'variants' | 'personalize' | 'analyze' | 'test';
};
...
export const contentGeneratorTool = tool({
  ...
  parameters: z.object({
    ...
    action: z.enum(['generate', 'optimize', 'variants', 'personalize', 'analyze', 'test']).describe('Action to perform')
  }),
  strict: true,
  execute: async (params: ContentGeneratorParams): Promise<any> => {
    ...
    if (contentResult.success && contentResult.data) {
      ...
      try {
        ...
        // Continue without image plan - Design Specialist will handle planning
      }
    }
  ...
  execute: async (params: EmailRendererParams): Promise<any> => {
    ...
    if (contentData.image_plan && typeof contentData.image_plan === 'string') {
      try {
        ...
        // Use general tags if none found
