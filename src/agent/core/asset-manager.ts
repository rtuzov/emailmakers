...
Пример ответа: ["путешествия", "заяц", "счастье", "отдых", "авиация"]`;
  ...
  private async searchExternalImages(tags: string[], params: AssetSearchParams, contentPackage?: any): Promise<StandardAsset[]> {
    ...
    // ❌ REMOVED MOCK EXTERNAL IMAGES - NO FALLBACK ALLOWED
    // This function should not generate mock/hardcoded images
    // External images must be generated by AI in asset-collection.ts
    
    console.log('⚠️ AssetManager does not generate external images - delegating to AI asset collection');
