...
async function generateSimpleAssetManifest(campaignPath: string, _strategy: any, destination: string) {
  ...
  try {
    ...
    const result = JSON.parse(resultString);
      ...
      // ✅ LOG TO CAMPAIGN
      logToFile('info', `Asset manifest generated: ${result.manifest?.images?.length || 0} images, ${result.manifest?.icons?.length || 0} icons`, 'ContentSpecialist-AssetStrategy');
...
ДОПОЛНИТЕЛЬНЫЙ КОНТЕКСТ И ДАННЫЕ ДЛЯ КОНТЕНТА:
...
${dateAnalysis ? `- Сезонные факторы: ${dateAnalysis.seasonal_factors || 'стандартные'}` : ''}
${pricingAnalysis ? `- Лучшая цена: ${pricingAnalysis.best_price || pricingAnalysis.optimal_dates_pricing?.cheapest_on_optimal || 'конкурентная'} ${pricingAnalysis.currency || 'RUB'}` : ''}
${pricingAnalysis ? `- Средняя цена: ${pricingAnalysis.optimal_dates_pricing?.average_on_optimal || pricingAnalysis.comprehensive_pricing?.average_price_overall || ''} ${pricingAnalysis.currency || 'RUB'}` : ''}
${pricingAnalysis ? `- Экономия: ${pricingAnalysis.optimal_dates_pricing?.average_on_optimal && pricingAnalysis.optimal_dates_pricing?.cheapest_on_optimal ? (pricingAnalysis.optimal_dates_pricing.average_on_optimal - pricingAnalysis.optimal_dates_pricing.cheapest_on_optimal).toLocaleString('ru-RU') : 'высокая'} ₽` : ''}
${pricingAnalysis ? `- Всего предложений: ${pricingAnalysis.comprehensive_pricing?.total_offers_found || 'множество'} вариантов` : ''}
...
${assetStrategy ? `- Ключевые сообщения: ${assetStrategy.content_strategy?.key_messages?.join(', ') || 'стандартные'}` : ''}

📊 АНАЛИЗ РЫНКА И ТРЕНДОВ:
${contextAnalysis?.market_trends ? `- Рыночные тренды: ${contextAnalysis.market_trends}` : ''}
${contextAnalysis?.consumer_behavior ? `- Поведение потребителей: ${contextAnalysis.consumer_behavior}` : ''}
${contextAnalysis?.seasonal_factors ? `- Сезонные факторы: ${contextAnalysis.seasonal_factors}` : ''}

🎯 ТАРГЕТИНГ И МОТИВАЦИИ:
${assetStrategy?.targeting_insights?.audience_motivations ? `- Мотивации аудитории: ${assetStrategy.targeting_insights.audience_motivations.join(', ')}` : ''}
${assetStrategy?.targeting_insights?.pain_points ? `- Болевые точки: ${assetStrategy.targeting_insights.pain_points.join(', ')}` : ''}
${assetStrategy?.targeting_insights?.decision_factors ? `- Факторы решений: ${assetStrategy.targeting_insights.decision_factors.join(', ')}` : ''}

🎯 КРИТИЧЕСКИ ВАЖНО - ВЫСОКОКОНВЕРТИРУЮЩИЙ КОНТЕНТ:

1. **ОБЯЗАТЕЛЬНО СОЗДАВАЙ PREHEADER** - критично для Gmail/Outlook!
2. **КОНКРЕТНЫЕ ПРЕИМУЩЕСТВА** - никаких абстрактных фраз!
3. **РАБОЧИЕ CTA ССЫЛКИ** - используй реальные URL, НЕ href="#"
4. **УСИЛЕННАЯ ПЕРСОНАЛИЗАЦИЯ** - больше чем просто [Имя]
5. **СИЛЬНЫЙ SOCIAL PROOF** - с цифрами и отзывами

ПРИМЕРЫ ХОРОШЕГО КОНТЕНТА:

📧 SUBJECT LINES (с эмодзи и ценой):
...
📱 PREHEADER (обязательно!):
✅ "Спецпредложение на Бали от 45,899₽! Только до 31 января"
✅ "🏖️ Тайланд от 38,999₽ • Вылеты из Москвы • Без виз"

💎 КОНКРЕТНЫЕ ПРЕИМУЩЕСТВА (используй РЕАЛЬНЫЕ данные!):
✅ "💰 Экономия до [реальная экономия из данных]₽ на билетах!"
...
✅ "📊 Более [количество предложений] вариантов на выбор"
✅ "🌟 Экологичный туризм" (если есть эко-тренды в данных)
❌ "Эмоциональное обогащение от новых знакомств" (абстрактно!)

🎯 SOCIAL PROOF (с цифрами):
✅ "★★★★★ 'Купили билеты через Kupibilet - сэкономили 12,000₽!' - Анна М., Москва"
✅ "🎯 За последний месяц 1,247 россиян купили билеты в ${params.destination}"
✅ "💎 Более 50,000 довольных клиентов выбрали нас в 2025 году"
❌ "Тысячи довольных путешественников уже выбрали..." (слабо!)

🔗 РАБОЧИЕ CTA (не href="#"!):
✅ href="/booking/${params.destination.toLowerCase()}?utm_source=email&utm_campaign=autumn"
✅ href="/destinations/${params.destination.toLowerCase()}?promo=special25"
✅ href="/deals/special-offers?destination=${params.destination.toLowerCase()}"

👤 РАСШИРЕННАЯ ПЕРСОНАЛИЗАЦИЯ:
✅ "Привет, [Имя]! Специально для вас скидка 5% на ${params.destination}"
✅ "Из [Город] в ${params.destination} от [Цена]₽"
✅ "Похоже, вас интересуют [Категория] путешествия"
✅ "Рекомендуем забронировать до [Дата] для лучшей цены"

🔥 ЭМОДЗИ И АКЦЕНТЫ:
🔥 ГОРЯЩЕЕ ПРЕДЛОЖЕНИЕ | ⏰ Осталось 3 дня | 💎 ЭКСКЛЮЗИВНО для подписчиков
🎯 ЛУЧШАЯ ЦЕНА ГАРАНТИРОВАНА | ✈️ ПРЯМЫЕ РЕЙСЫ | 🏖️ ПЛЯЖНЫЙ ОТДЫХ

ТРЕБОВАНИЯ:
1. Создай subject line с эмодзи и ценой
2. ОБЯЗАТЕЛЬНО создай preheader - не пропускай!
3. Создай сильный заголовок (headline)
4. Напиши основной текст с эмоциональными триггерами
5. Добавь КОНКРЕТНЫЕ преимущества (не абстрактные!)
6. Включи СИЛЬНЫЕ элементы социального доказательства с цифрами
7. Создай ощущение срочности и дефицита
8. Используй РАБОЧИЕ CTA ссылки (не href="#")

ОТВЕТ ДОЛЖЕН БЫТЬ СТРОГО В JSON БЕЗ MARKDOWN БЛОКОВ:

{
  "subject_line": {
    "primary": "тема с эмодзи и ценой (пример: 🇹🇭 Тайланд от 38,999₽! Спецпредложение до 30 января)",
    "alternative": "альтернативная тема с акцентом на скидку",
    "personalization": "элементы персонализации с [Имя], [Город], [Дата]"
  },
  "preheader": "ОБЯЗАТЕЛЬНЫЙ текст превью письма 50-90 символов с ценой и срочностью",
  "headline": {
    "main": "основной заголовок с эмодзи и направлением",
    "subheadline": "подзаголовок с ценой и выгодой"
  },
  "body": {
    "opening": "вступительный абзац с эмоциональным крючком",
    "main_content": "основное содержание с конкретными датами и ценами",
    "benefits": [
      "💰 Экономия до X₽ на билетах (КОНКРЕТНО!)",
      "✈️ Прямые рейсы без пересадок из Москвы",
    ...
    "social_proof": "★★★★★ Отзыв с именем и городом + статистика с цифрами (например: 1,247 россиян купили билеты в направление за месяц)",
    "urgency_elements": "⏰ Конкретная срочность: до какой даты, сколько мест осталось",
    "closing": "заключительный абзац с призывом не упустить возможность"
  },
  "call_to_action": {
    "primary": {
      "text": "Купить билеты за X₽ ✈️",
      "url": "/booking/${params.destination.toLowerCase()}?utm_source=email&utm_campaign=special"
    },
    "secondary": {
      "text": "Посмотреть все рейсы 🔍",
      "url": "/destinations/${params.destination.toLowerCase()}?promo=special25"
    },
    "urgency_cta": {
      "text": "⚡ Успеть забронировать до [дата]!",
      "url": "/deals/special-offers?destination=${params.destination.toLowerCase()}"
    }
  },
  "personalization": {
    "greeting": "Привет, [Имя]! Специально для вас скидка 5% на ${params.destination}",
    "content_adaptation": "Из [Город] в ${params.destination} от [Цена]₽",
    "recommendations": "Рекомендуем забронировать до [Дата] для лучшей цены в ${params.destination}"
  },
  "emotional_hooks": {
    "desire": "Представьте себя на пляжах ${params.destination} уже через месяц",
    "fear_of_missing_out": "Места по этой цене быстро заканчиваются - за вчера осталось только 12 мест",
    "aspiration": "Осуществите мечту о путешествии в ${params.destination} с максимальной выгодой"
  },
  "content_structure": {
    "flow": "urgency_early → benefits → content → social_proof → primary_cta → personalization",
    "key_messages": [
      "Лучшая цена на ${params.destination} с ограниченным временем",
      "Конкретные выгоды для клиентов Kupibilet",
      "Социальное доказательство успешности"
    ],
    "conversion_path": "эмоциональный крючок → срочность → выгоды → доверие → действие"
  },
  "compliance": {
    "legal_disclaimers": "Цены указаны на момент публикации и могут изменяться",
    "unsubscribe": "Чтобы отписаться от рассылки, нажмите сюда",
    "privacy": "Мы защищаем ваши данные согласно политике конфиденциальности"
  }
}
