...
Затем выполните остальные 8 инструментов подряд:
2. `dateIntelligence` 3. `pricingIntelligence` 4. `assetStrategy` 5. `contentGenerator` 6. `generateAssetManifest` 7. `generateTechnicalSpecification` 8. `finalizeContentAndTransferToDesign` 9. `transferToDesignSpecialist`
...
**ПОСЛЕ ЗАВЕРШЕНИЯ** `contextProvider` вызовите остальные 8 инструментов подряд:
...
6. `generateAssetManifest` - Создание манифеста ассетов
7. `generateTechnicalSpecification` - Создание технической спецификации
8. `finalizeContentAndTransferToDesign` - Финализация контекста
...
**ВЫПОЛНИТЕ ВСЕ 9 ИНСТРУМЕНТОВ ПОДРЯД:**
...
6. `generateAssetManifest` - Сразу после пятого
7. `generateTechnicalSpecification` - Сразу после шестого
8. `finalizeContentAndTransferToDesign` - Сразу после седьмого
9. `transferToDesignSpecialist` - ФИНАЛЬНЫЙ ОБЯЗАТЕЛЬНЫЙ!

**НЕ ОСТАНАВЛИВАЙТЕСЬ** после одного инструмента - выполняйте все 9 подряд!
