...
**ПОСЛЕ ВЫПОЛНЕНИЯ ВСЕХ 8 ИНСТРУМЕНТОВ** обязательно выполните handoff к Design Specialist:

🚨 **КРИТИЧЕСКИ ВАЖНО**: После вызова `finalizeContentAndTransferToDesign` **НЕМЕДЛЕННО** вызовите handoff к Design Specialist:

```
Transfer to Design Specialist with the complete context and generated content.
```

**ОБЯЗАТЕЛЬНЫЙ ПОРЯДОК ЗАВЕРШЕНИЯ:**
1. Вызвать `finalizeContentAndTransferToDesign`
2. **НЕМЕДЛЕННО** выполнить handoff к Design Specialist
3. Передать сообщение: "Continue with email design using generated content, technical specification, and asset manifest"

**БЕЗ HANDOFF DESIGN SPECIALIST НЕ ПОЛУЧИТ УПРАВЛЕНИЕ!**