...
describe('OptimizationEngine', () => {
  ...
  beforeEach(() => {
    ...
    mockAnalyzer = {
      ...
      analyzeErrorPatterns: vi.fn(),
      predictPerformanceIssues: vi.fn(),
  ...
  describe('System Performance Analysis', () => {
    test('should analyze system performance successfully', async () => {
      // Setup mocks
      mockAnalyzer.analyzePerformanceTrends = vi.fn().mockResolvedValue([
        {
          metric_name: 'system_success_rate',
          trend_direction: 'up',
          change_percent: 3.5,
      ...
      mockAnalyzer.identifyBottlenecks = vi.fn().mockResolvedValue([
        {
          id: 'test-bottleneck',
          type: 'memory',
          affected_agent: 'design-specialist',
      ...
      mockAnalyzer.analyzeErrorPatterns = vi.fn().mockResolvedValue([]);
      mockAnalyzer.predictPerformanceIssues = vi.fn().mockResolvedValue([]);
    ...
    test('should handle analysis errors gracefully', async () => {
      mockAnalyzer.analyzePerformanceTrends = vi.fn().mockRejectedValue(new Error('Analysis failed'));
  ...
  describe('Optimization Recommendations', () => {
    test('should generate optimization recommendations', async () => {
      ...
      vi.spyOn(engine, 'analyzeSystemPerformance').mockResolvedValue(createMockSystemAnalysis());
    ...
    test('should respect max concurrent optimizations limit', async () => {
      ...
      vi.spyOn(engine, 'analyzeSystemPerformance').mockResolvedValue(analysisWithManyBottlenecks);
  ...
  describe('Optimization Application', () => {
    ...
    test('should handle optimization failures gracefully', async () => {
      ...
      vi.spyOn(engine as any, 'executeOptimization').mockRejectedValue(new Error('Execution failed'));
  ...
  describe('Monitoring and Tracking', () => {
    ...
    test('should handle tracking errors gracefully', async () => {
      ...
      vi.spyOn(engine as any, 'checkRollbackTriggers').mockRejectedValue(new Error('Tracking failed'));
  ...
  describe('Integration Points', () => {
    test('should handle missing metrics gracefully', async () => {
      ...
      vi.spyOn(engine as any, 'getCurrentMetrics').mockResolvedValue({
        timestamp: new Date().toISOString(),
        agent_metrics: {},
        system_metrics: {
          total_requests: 0,
  ...
  describe('Error Handling', () => {
    test('should handle analyzer failures', async () => {
      mockAnalyzer.analyzePerformanceTrends = vi.fn().mockRejectedValue(new Error('Analyzer failed'));

      await expect(engine.analyzeSystemPerformance()).rejects.toThrow('System analysis failed');
    });

    test('should handle recommendation generation failures', async () => {
      vi.spyOn(engine, 'analyzeSystemPerformance').mockRejectedValue(new Error('Analysis failed'));
    ...
    test('should continue processing other optimizations when one fails', async () => {
      ...
      vi.spyOn(engine as any, 'executeOptimization')
